...
function scoreflash()
{
  ...
  score = (60*correct - Math.floor(parseFloat(totaltime))- 5*incorrect)
  document.scoreform.score.value = score;
  document.getElementById("l0").innerHTML = 'Score:'+score;
...
function resetPage(dataArray)
{
    ...
    if ((incorrect == 5) || (asked.length == dataArray.length) || (correct+incorrect == 10))
        scoreflash()
    ...
    $(document.body).click(function(evt){
        ...
        if((currentID[0] == "x")&&(document.getElementById(currentID).style.background == "none repeat scroll 0% 0% rgba(102, 153, 147, 0.5)"))
        {
            ...
            if(timesClicked == 9)
            {
                ...
                totaltime = parseFloat(totaltime) + parseFloat(parseFloat(time).toFixed(2));
                console.log(totaltime)
                time = 0
                $(document.body).off('click');
                resetPage(dataArray)
